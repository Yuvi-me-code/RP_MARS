...
def features_to_dataframe(features_array):
    """Convert feature array to DataFrame using saved feature names."""
    try:
        with open(r'feature_names.txt', 'r') as f:
...
def predict_terrain(image):
    """Extracts features, creates DataFrame, and predicts the terrain type."""
    ...
    class_pred = model.predict(feature_df)[0]
    prob = model.predict_proba(feature_df).max()
