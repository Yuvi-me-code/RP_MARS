...
def objective(trial):
    ...
    if model_name == "XGBoost":
        ...
        model = CatBoostClassifier(
            ...
            grow_policy="Depthwise",  # Alternative to SymmetricTree
            verbose=0,
            random_state=42
        )
...
xgb_model = XGBClassifier(**best_params, use_label_encoder=False, eval_metric="mlogloss", tree_method='gpu_hist' if device == "cuda" else "auto", random_state=42)
lgbm_model = LGBMClassifier(**best_params, device="gpu" if device == "cuda" else "cpu", random_state=42)
cat_model = CatBoostClassifier(**best_params, task_type="GPU" if device == "cuda" else "CPU", grow_policy="Depthwise", verbose=0, random_state=42)
...
# Save the best model
model_filename = "final_best_stacked_model.pkl"
with open(model_filename, "wb") as model_file:
    pickle.dump(stacked_model, model_file)
print("Best model saved as 'final_best_stacked_model.pkl'")
